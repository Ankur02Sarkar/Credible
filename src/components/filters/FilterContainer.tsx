...
import type { FilterOptions, AppliedFilter } from "~/types/credit-card";
...
export function FilterContainer({
  ...
  const updateFilter = useCallback(
    (key: Exclude<keyof FilterOptions, 'page'>, value: string) => {
      ...
      if (value && value !== "") {
        ...
        switch (key) {
          case 'sortby':
            label = SORT_OPTIONS.find(opt => opt.value === value)?.label || value;
            break;
          case 'privileges':
            label = PRIVILEGE_OPTIONS.find(opt => opt.value === value)?.label || value;
            break;
          case 'emptype':
            label = EMPLOYMENT_OPTIONS.find(opt => opt.value === value)?.label || value;
            break;
          case 'incomeRange':
            label = INCOME_OPTIONS.find(opt => opt.value === value)?.label || value;
            break;
        }
  ...
  const removeFilter = useCallback((type: AppliedFilter['type']) => {
    updateFilter(type, "");
  }, [updateFilter]);
  ...
  return (
    <div className={cn("space-y-6", className)}>
      ...
      <div className={cn(
        ...
        glassGradients.card,
        "shadow-2xl border border-white/10 dark:border-white/5"
      )}>
